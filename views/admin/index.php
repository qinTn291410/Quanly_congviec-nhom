...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
    <h1 style="font-size: 1.8rem; margin: 0; color: #37352f;"><i class="fas fa-shield-alt" style="color: #eb3639;"></i> Quản trị Hệ thống (Admin)</h1>
</div>
        ...
        <table style="width: 100%; border-collapse: collapse; text-align: left;">
            <thead>
                <tr style="border-bottom: 2px solid #f0f0f0; color: #787774;">
                    ...
                    <th style="padding: 15px 10px;">Họ và Tên</th>
                    ...
                    <tr style="border-bottom: 1px solid #f0f0f0;">
                        ...
                        <td style="padding: 15px 10px;">
                            <?php if ($u['id'] == $_SESSION['user_id']): ?>
                                <span style="background: #fde8e8; color: #eb3639; padding: 4px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold;">Admin (Bạn)</span>
                            <?php else: ?>
                        ...
                        <td style="padding: 15px 10px;">
                            <?php if ($u['is_locked']): ?>
                                <span style="background: #f1f1f0; color: #787774; padding: 4px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold;"><i class="fas fa-lock"></i> Đã khóa</span>
                            <?php else: ?>
                                <span style="background: #dbeddb; color: #0f7b6c; padding: 4px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold;"><i class="fas fa-check"></i> Hoạt động</span>
                            <?php endif; ?>
                        </td>
                        <td style="padding: 15px 10px; text-align: right;">
                            <?php if ($u['id'] != $_SESSION['user_id']): ?>
                                <a href="index.php?action=admin-toggle-lock&id=<?= $u['id'] ?>" style="background: <?= $u['is_locked'] ? '#0f7b6c' : '#eb3639' ?>; color: white; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 0.85rem; font-weight: 500;">
                                    <?= $u['is_locked'] ? 'Mở khóa' : 'Khóa tài khoản' ?>
                                </a>
                            <?php endif; ?>
                        </td>
                    </tr>
...
<?php else: ?>
    <div style="background: white; padding: 30px; border-radius: 12px; border: 1px solid #e3e2e0; box-shadow: 0 4px 15px rgba(0,0,0,0.03); max-width: 800px;">
        <form action="index.php?action=admin-save-config" method="POST">
            <h3 style="margin: 0 0 20px 0; color: #0f7b6c;"><i class="fas fa-tags"></i> 1. Danh mục & Nhãn (Cách nhau bằng dấu phẩy)</h3>
            
            <div style="margin-bottom: 15px;">
                <label style="display: block; font-weight: 500; margin-bottom: 5px;">Trạng thái Task (Statuses)</label>
            ...
            <div style="margin-bottom: 15px;">
                <label style="display: block; font-weight: 500; margin-bottom: 5px;">Mức độ ưu tiên (Priorities)</label>
            ...
            <div style="margin-bottom: 30px;">
                <label style="display: block; font-weight: 500; margin-bottom: 5px;">Nhãn công việc (Labels)</label>
            ...
            <h3 style="margin: 0 0 20px 0; color: #2383e2;"><i class="fas fa-envelope"></i> 2. Cấu hình Email (SMTP)</h3>
            
            <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                <div style="flex: 2;">
                    <label style="display: block; font-weight: 500; margin-bottom: 5px;">Máy chủ SMTP (Host)</label>
                ...
                <div style="flex: 1;">
                    <label style="display: block; font-weight: 500; margin-bottom: 5px;">Cổng (Port)</label>
